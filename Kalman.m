...
data = xlsread('C:\Users\Asus\Desktop\MPU-DATA\10.csv');
velocity = data(:,3)/1000000;

%%
figure;
h = histogram(velocity);
mu=mean(velocity)
sd = std(velocity)
% %Overlay the mean
% hold on
...
Xhawa = velocity' .* dt
Xfalse = zeros(1,Nsamples);
for l=2:Nsamples
    Xfalse(l) = Xfalse(l-1) + dt * (velocity(l-1));
end
figure;
hold on;
grid on;
...
% measurements
InstantaneousVelocity = [0 (Z_buffer(2:Nsamples)-Z_buffer(1:Nsamples-1))/dt];
...
plot(t,InstantaneousVelocityRunningAverage,'r');
